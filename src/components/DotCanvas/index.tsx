...
class DotCanvas extends React.Component {
  ...
  draw = () => {
    this.HomepageCanvasReq = requestAnimationFrame(
      this.draw
    )
    this.canvasRender()
  }
  ...
  canvasRender = () => {
    ...
    for (
      ...
      for (
        ...
        this.angleRad =
          ((this.angleDeg + this.angleDeltaXDeg) *
            Math.PI) /
          180

        var x =
          Math.cos(this.angleRad) * this.CIRCLE_RADIUS + i
        var y =
          Math.sin(this.angleRad) * this.CIRCLE_RADIUS + j
  ...
  render() {
    return (
      <div style={{...this.props.style, pointerEvents: 'none'}} ref="mainContainer">
