...
class Star extends React.Component {
  ...
  getPosition = () => {
    ...
    this.xPos = ((this.canvasHeight / this.canvasWidth) * this.canvasWidth * this.x) / this.z + this.canvasWidth / 2
    this.yPos = (this.canvasHeight * this.y) / this.z + this.canvasHeight / 2

    // Detect collision with black hole
    if (
      Math.sqrt((this.xPos - this.props.HOLE.x) * (this.xPos - this.props.HOLE.x) + (this.yPos - this.props.HOLE.y) * (this.yPos - this.props.HOLE.y)) <= this.props.HOLE.r ||
  ...
  draw() {
    ...
    this.props.ctx.globalAlpha = (this.props.Z_RANGE + this.z) / (this.props.Z_RANGE * 2)
...
class BlackHole extends React.Component {
  ...
  setSizes = () => {
    ...
    this.smallerSize = this.canvasWidth > this.canvasHeight ? this.canvasHeight : this.canvasWidth
  }
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < this.STARS_COUNT; i++) this.stars[i] = new Star({ctx: this.ctx, HOLE: this.HOLE, Z_RANGE: this.Z_RANGE, Z_VELOCITY: this.Z_VELOCITY, canvas: {canvasWidth: this.canvasWidth, canvasHeight: this.canvasHeight, smallerSize: this.smallerSize}})
  ...
  render() {
    return (
      <div style={{minHeight: '20rem', minWidth: '100%', ...this.props.style}}>
