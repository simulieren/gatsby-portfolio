        ...
        {this.state.textItems.map((text, i) => {
          return (
            <Transition
              ...
              from={{ opacity: 0, transform: 'translateY(-100%) rotateX(60deg)' }}
              enter={{ opacity: 1, transform: 'translateY(0%) rotateX(0deg)' }}
              leave={{ opacity: 0, transform: 'translateY(100%) rotateX(-60deg)' }}
            >
              {index => props =>
                index === i && (
                  <animated.div style={{ ...props, position: 'absolute', width: '100%' }} key={index}>
                    {text}
                  </animated.div>
                )}
            </Transition>
